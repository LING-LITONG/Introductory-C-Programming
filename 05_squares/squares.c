...
void squares(int size1, int x_offset, int y_offset, int size2) {
  ...
  for (int y = 0; y < h; y++) {
    ...
    for (int x = 0; x < w; x++) {
      ...
      // if so, print a *
      if ((isInRange(x, x_offset, x_offset + size2) && isAtBorder(y, y_offset, y_offset + size2 - 1)) ||
          (isInRange(y, y_offset, y_offset + size2) && isAtBorder(x, x_offset, x_offset + size2 - 1))) {
        printf("*");
      } 
      //if not,
      // check if EITHER
